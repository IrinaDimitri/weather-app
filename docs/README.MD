...
Использует PostgreSQL, Open-Meteo API, Swagger и поддерживает контейнеризацию через Docker.
...
4. [Работа с PostgreSQL в Docker](#работа-с-postgresql-в-docker)
   - [Проверка подключения](#проверка-подключения-к-postgresql)
   - [Навигация по БД](#навигация-и-просмотр-структуры-базы-данных)
...
- В случае запроса на **/ping** или **/health**, он обрабатывается backend, который отвечает на эти маршруты.
